...
import pageTitleData from '../../data/pageTitle.json';

const PageTitle = ({ pageTitle }) => {
  const [lang, setLang] = useState('en');
  const [text, setText] = useState(pageTitleData.en);

  useEffect(() => {
    const storedLang = localStorage.getItem('selectedLanguage') || 'en';
    setLang(storedLang);
    setText(pageTitleData[storedLang]);

    const handleLangChange = () => {
      const newLang = localStorage.getItem('selectedLanguage') || 'en';
      setLang(newLang);
      setText(pageTitleData[newLang]);
    };
